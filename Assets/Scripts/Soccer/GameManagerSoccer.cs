...
public class GameManagerSoccer : MonoBehaviour
{
    ...
    void OnGUI()
    {
        ...
        {
            StatusLabels(red_score, blue_score, team_name);
    ...
    static void StatusLabels(int redScore, int blueScore, string team_name)
    {
        ...
        {
            if (NetworkManager.Singleton.LocalClientId == 1) GUILayout.Label("Team: Blue");
            if (NetworkManager.Singleton.LocalClientId == 2) GUILayout.Label("Team: Red");
            GUILayout.Label("Team Name: " + team_name);
        }
    }
    ...
    {
        var vehicle = new GameObject();
        vehicle = Instantiate(vehicle_template, Vector3.zero, Quaternion.identity);
